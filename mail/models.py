...
class userMailAddress(models.Model):
    ...
    emailIntervalTime = models.IntegerField(db_column='emailIntervalTime', blank=False, default=60,
                                            help_text='邮件定时发送间隔时间（S）')
...
# 邮件附件
class emaileAttachment(models.Model):
    id = models.AutoField(db_column='ID', primary_key=True)
    emailFrom = models.EmailField(db_column='emailFrom', blank=False, null=False, help_text='发件人邮件')
    contentID = models.IntegerField(db_column='contentID', null=False, help_text='邮件内容ID')
    attFileName = models.CharField(db_column='attFileName', max_length=100, null=False, help_text='附件文件名称')
